    ...
    <div class="container mx-auto md:flex px-3 md:p-0 mt-10 gap-6">
        <div class="md:w-1/2">
            ...
            <div class="mt-3 flex gap-2">
                @auth
                    <form action="{{ $post->checkLike(auth()->user()) ? route('post.likes.destroy' , $post) : route('post.likes.store' , $post) }}" method="POST">
                        @csrf
                        @if ($post->checkLike(auth()->user()) )
                            @method('DELETE')
                        @endif
                        <button type="submit">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="{{ $post->checkLike(auth()->user()) ? "black" : "white" }}" viewBox="0 0 24 24" stroke-width="1.5"
